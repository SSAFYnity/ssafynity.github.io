...
function FilterDropdown({ filter, onChange }: { filter: Filter; onChange: (v: Filter) => void }) {
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        onClick={() => setOpen(o => !o)}
        className={`min-w-36 whitespace-nowrap flex items-center justify-between gap-1.5 pl-3.5 pr-2.5 py-2 sm:py-1.5 rounded-full text-sm sm:text-xs font-black border transition-colors ${
          active
        ...
        <div className="absolute top-full left-0 mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg z-20 w-[min(20rem,calc(100vw-2rem))] sm:w-max py-1">
          {FILTERS.map(f => (
            <button
              ...
              className={`flex items-center gap-2 w-full px-3.5 py-2 text-xs hover:bg-slate-50 transition-colors whitespace-normal sm:whitespace-nowrap ${
      ...
      {isFull && club.achievedAt ? (
        <div className="flex flex-col gap-1 border-t border-slate-200 pt-3 -mt-1">
          <p className="text-xs text-slate-500 leading-relaxed break-keep">
            최소 인원이 모였습니다. 구성원 논의 후 정식 동아리 전환을 준비합니다.
          </p>
    ...
    <div className="relative" style={wrapperStyle}>
      ...
      >
        ...
        <div className="relative" style={{ minHeight: DESC_3LINE_HEIGHT }}>
          <p
            ref={descRef}
            className={`text-sm text-slate-600 leading-relaxed break-keep text-pretty ${
              expanded ? '' : 'line-clamp-3'
            }`}
          >{expanded ? <ResponsiveText text={club.shortDesc} /> : collapseNewlines(club.shortDesc)}</p>
          <p ref={measureRef} aria-hidden className="text-sm leading-relaxed break-keep text-pretty absolute top-0 left-0 right-0 invisible pointer-events-none"><ResponsiveText text={club.shortDesc} /></p>
        </div>
    ...
    <div className="flex flex-col">
          ...
          >
            ...
            <div className="flex items-end justify-between gap-4">
              <p className="text-sm text-slate-500 leading-relaxed break-keep text-pretty max-w-xl">
                아직 인원이 부족하지만 동아리를 만들고 싶은 동문들의 모집 공간입니다. 5인 이상이 모이면 SSAFYnity가 단체 채팅방을 개설하고 정식 동아리 전환을 안내합니다.
              </p>
              <Link
                to="/clubs/apply"
      ...
      <section className="bg-slate-50 pt-8 pb-20 lg:pt-10">
          ...
          {allPreClubs.length === 0 ? (
            ...
            <>
              {/* 필터 + 검색 + 정렬 */}
              <div className="sticky sm:static top-[calc(4.5rem+env(safe-area-inset-top))] z-40 mb-4">
                <div className="bg-white border border-slate-200 rounded-2xl px-4 py-3 shadow-sm sm:shadow-none">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="w-full">
                      <SearchBar value={query} onChange={setQuery} onSubmit={submitSearch} placeholder="소개 검색" />
                    </div>
                  </div>
                </div>
              </div>
